...
export async function Footer() {
  ...
  return (
    <footer className="mt-10 bg-navy text-white">
      <div className="h-1 bg-gradient-to-r from-accent via-amber-300 to-accent-dark" />
      <Container className="grid grid-cols-1 gap-10 py-16 sm:grid-cols-2 md:grid-cols-4">
        ...
        <div>
          ...
          <ul className="mt-4 space-y-2 text-sm">
            <li><Link href="/usluge" className="hover:text-accent">Sve usluge</Link></li>
      ...
      <div className="border-t border-white/10">
        <Container className="flex flex-col items-center justify-between gap-2 py-6 text-xs text-white/50 sm:flex-row">
          ...
          {settings.emergencyAvailability && <span>{settings.emergencyAvailability}</span>}
        </Container>
      </div>
    </footer>
  );
